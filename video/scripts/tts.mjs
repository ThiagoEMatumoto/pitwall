...
// so imprime o plano. E idempotente por sha256(text + voiceId + modelId +
// voiceSettings): cena cujo hash bate com o manifesto e cujo mp3 existe no disco
// nao e regerada.
//
// Uso:
...
async function synthesize(text, voiceId, modelId, voiceSettings, key) {
  const url =
    `https://api.elevenlabs.io/v1/text-to-speech/${voiceId}/stream` +
    "?output_format=mp3_44100_128";

  const body = { text, model_id: modelId };
  // Omitido quando o script.json nao declara nada: o ElevenLabs cai nos ajustes
  // salvos na propria voz, que e o comportamento antigo deste script.
  ...
  try {
    ...
    throw new Error(
      `nao consegui chamar o servico de voz (rede ou tempo esgotado): ${err.message}`,
    );
  }
  ...
  if (res.status !== 200) {
    ...
    throw new Error(
      `o servico de voz respondeu HTTP ${res.status}${pista ? ": " + pista : ""}`,
    );
  }

  const bytes = Buffer.from(await res.arrayBuffer());
  if (bytes.length === 0)
    throw new Error("o servico de voz devolveu um arquivo vazio");
  return bytes;
}
...
async function probeDurationSec(absPath) {
  ...
  if (!Number.isFinite(seconds) || seconds <= 0) {
    throw new Error(
      `ffprobe nao devolveu duracao valida para ${absPath}: ${stdout.trim()}`,
    );
  }
  return seconds;
}
...
/**
 * Impressao digital estavel dos voice_settings. Entra no hash porque os ajustes
 * mudam o AUDIO sem mudar uma letra do texto: sem isto, alterar a expressividade
 * no script.json deixaria o cache achando que ja gerou e pularia as 16 cenas.
 * Chaves ordenadas para que reordenar o JSON a mao nao invalide o cache a toa.
 * String vazia quando nao ha ajustes = hash identico ao esquema antigo.
 */
const voiceSettingsFingerprint = (voiceSettings) => {
  if (!voiceSettings) return "";
  const sorted = Object.keys(voiceSettings)
    .sort()
    .map((key) => [key, voiceSettings[key]]);
  return JSON.stringify(sorted);
};

const textHashOf = (text, voiceId, modelId, voiceSettings) =>
  createHash("sha256")
    .update(text + voiceId + modelId + voiceSettingsFingerprint(voiceSettings))
    .digest("hex");
...
function buildPlan(script, locales, previous) {
  ...
  const voiceSettings = script.tts.voiceSettings ?? null;
  const plan = [];
  for (const locale of locales) {
    const voice = script.tts.voices[locale];
    if (!voice)
      throw new Error(
        `script.json nao tem voz configurada para o locale ${locale}`,
      );
    for (const scene of script.scenes) {
      const text = scene.narration[locale];
      if (!text)
        throw new Error(`cena ${scene.id} nao tem narracao para ${locale}`);
      const hash = textHashOf(text, voice.voiceId, modelId, voiceSettings);
...
function printPlan(plan, locales, go) {
  const header = go
    ? "Plano de sintese (--go: VAI CHAMAR A API)"
    : "Plano de sintese (DRY-RUN — nenhuma chamada a API)";
  console.log(header);
  let billed = 0;
  for (const locale of locales) {
    ...
    console.log(
      `    voice_settings: ${voice.voiceSettings ? JSON.stringify(voice.voiceSettings) : "(nenhum — default da voz)"}`,
    );
...
async function writeManifest(script, previous) {
  ...
  for (const locale of allLocales) {
    ...
    for (const row of full.filter((r) => r.locale === locale)) {
      ...
      const durationSec = row.onDisk
        ? await probeDurationSec(row.absPath)
        : row.targetSec;
      scenes.push({
        id: row.id,
  ...
  // voiceSettings fica no topo por proveniencia: e o unico jeito de ler, olhando
  // so o manifesto, com que expressividade os mp3 no disco foram sintetizados.
  const manifest = {
    version: 1,
    fps: script.fps,
    voiceSettings: script.tts.voiceSettings ?? null,
    locales,
  };
...
async function main() {
  ...
  for (const locale of locales) {
    if (!script.locales.includes(locale)) {
      console.error(
        `locale desconhecido: ${locale} (disponiveis: ${script.locales.join(", ")}, all)`,
      );
      process.exit(1);
    }
  }
  ...
  if (args.checkKey) {
    ...
    console.log(
      `credencial OK — origem: ${key.source}, ${key.value.length} caracteres.`,
    );
    return;
  }
  ...
  for (const row of plan) {
    ...
    console.log(
      `  ok     ${row.locale}/${row.id}  ${(bytes.length / 1024).toFixed(0)} KB  ${seconds.toFixed(2)}s`,
    );
  }
