...
const EMPTY: DecodedSecrets = { values: {}, unreadable: [], plaintext: [], legacy: false }
...
function decodeLegacy(raw: unknown): DecodedSecrets {
  ...
  return { values, unreadable: [], plaintext, legacy: true }
}
...
export function decodeSecrets(raw: unknown, crypto: SecretCrypto): DecodedSecrets {
  ...
  return { values, unreadable, plaintext, legacy: false }
}
