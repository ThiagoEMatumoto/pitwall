...
import {
  ...
  type SpawnHandoffChildInput,
} from "../handoff/spawn-child";
...
describe("mcp tools — objectives/KRs", () => {
  ...
  it("objective_create rejeita input inválido (zod)", () => {
    expect(() =>
      tool("objective_create").handler({ title: "", kind: "okr" }),
    ).toThrow();
    expect(() =>
      tool("objective_create").handler({ title: "X", kind: "nope" }),
    ).toThrow();
  ...
  it("objective_list filtra e objective_get retorna detalhe com KRs", () => {
    ...
    const { objective: detail } = call<{ objective: ObjectiveDetail }>(
      "objective_get",
      {
        id: objective.id,
      },
  ...
  it("objective_update muda só os campos enviados", () => {
    ...
    const { objective: updated } = call<{ objective: Objective }>(
      "objective_update",
      {
        id: objective.id,
        title: "Depois",
  ...
  it("key_result_update altera o KR e broadcasta o marcador", () => {
    ...
    const { keyResult: updated } = call<{ keyResult: KeyResult }>(
      "key_result_update",
      {
        id: keyResult.id,
        title: "KR renomeado",
...
describe("mcp tools — tasks", () => {
  ...
  it("task_set_links substitui vínculos e notifica quem ganhou E quem perdeu", () => {
    const { objective: a } = call<{ objective: Objective }>(
      "objective_create",
      {
        title: "Perde tarefa",
        kind: "okr",
    ...
    const { objective: b } = call<{ objective: Objective }>(
      "objective_create",
      {
        title: "Ganha tarefa",
        kind: "okr",
...
describe("mcp tools — features", () => {
  ...
  it("feature_set_objective_links notifica objetivos que ganharam e perderam", () => {
    ...
    const { feature: relinked } = call<{ feature: Feature }>(
      "feature_set_objective_links",
      {
        featureId: feature.id,
...
describe("mcp tools — scheduled jobs", () => {
  ...
  it("scheduled_job_create/update rejeitam permissionMode autônomo (gate observe-only)", () => {
    ...
    const { job: updated } = call<{ job: ScheduledJob }>(
  ...
  it("scheduled_job_update pausa o job (enabled=false → row enabled=0)", () => {
    ...
    const { job: paused } = call<{ job: ScheduledJob }>(
      "scheduled_job_update",
      {
        id: job.id,
        enabled: false,
  ...
  it("job_report lança quando o runId não existe", () => {
    expect(() =>
      tool("job_report").handler({ runId: "nao-existe", report: "x" }),
    ).toThrow();
  });
});
...
describe("composeJobKickoff (delta-via-prompt)", () => {
  ...
  it("critique NÃO injeta o playbook de browser", () => {
    expect(composeJobKickoff({ prompt: "roda" })).not.toContain(
      "Playbook de auditoria web",
    );
    expect(
      composeJobKickoff({ prompt: "roda", kind: "critique" }),
    ).not.toContain("Playbook de auditoria web");
  });
});
...
describe("mcp tools — session_handoff sem gate", () => {
  ...
  describe("identidade da sessão-mãe (ctx)", () => {
    ...
    function callAs<T>(
      ...
      const def = buildTools(notify, { motherSessionId }).find(
        (t) => t.name === name,
      );
      if (!def) throw new Error(`tool not registered: ${name}`);
...
describe("mcp tools — content contracts", () => {
  ...
  function novoContrato(slug: string): ContentContract {
    const { contract } = call<{ contract: ContentContract }>(
  ...
  it("content_contract_get por slug devolve o contrato inteiro e o changelog", () => {
    ...
    expect(contract.forbiddenFacts[0].neutralForm).toBe(
      "o BPC é revisto periodicamente",
    );
  ...
  it("content_gate_run_list filtra por contrato e gate, mais recente primeiro", () => {
    ...
    const { items } = call<{ items: ContentGateRun[] }>(
...
describe("mcp tools — diagrams", () => {
  ...
  it("diagram_create converte skeleton, persiste, broadcasta e devolve skeleton derivado", () => {
    ...
    const row = getDb()
      .prepare(
        "SELECT source_format, source, version FROM diagrams WHERE id = ?",
      )
      .get(diagram.id) as {
  ...
  it("diagram_patch aplica ops sobre a cena vigente e grava snapshot", () => {
    ...
    const version = getDb()
      .prepare(
        "SELECT author, summary FROM diagram_versions WHERE diagram_id = ? AND version = 2",
      )
  ...
  it("diagram_delete recusa diagrama ativo (two-step) e apaga após archive", () => {
    ...
    expect(
      getDb()
        .prepare("SELECT COUNT(*) AS n FROM diagrams WHERE id = ?")
        .get(diagram.id),
    ).toEqual({ n: 1 });
  ...
  it("integração: create (4 nós/3 setas) → get → patch (move + rename) → archive → delete", () => {
    ...
    const patched = call<{ diagram: DiagramMetaOut; skeleton: SkeletonFull[] }>(
      "diagram_patch",
      {
        id: diagram.id,
    ...
    const elements = full.diagram.scene.elements as Array<
    ...
    const etlLabel = elements.find((el) => el.id === "etl__label") as Record<
      string,
      unknown
    >;
    ...
    expect(
      db
        .prepare(
          "SELECT COUNT(*) AS n FROM diagram_versions WHERE diagram_id = ?",
        )
        .get(diagram.id),
    ).toEqual({ n: 0 });
    expect(
      db
        .prepare("SELECT COUNT(*) AS n FROM diagram_links WHERE diagram_id = ?")
        .get(diagram.id),
    ).toEqual({ n: 0 });
  });
});
...
describe("mcp tools — diagram library", () => {
  ...
  it("diagram_library_install via library_json persiste, broadcasta e devolve metas", async () => {
    const out = (
      await tool("diagram_library_install").handler({ library_json: LIB_V2 })
    ...
    const rows = getDb()
      .prepare(
        "SELECT id, status FROM diagram_library_items ORDER BY position ASC",
      )
  ...
  it("reinstalar a mesma biblioteca faz merge por id (não duplica)", async () => {
    ...
    const out = (
      await tool("diagram_library_install").handler({ library_json: renamed })
    ).structuredContent as { added: number; items: LibraryItemMetaOut[] };
  ...
  it("diagram_library_list devolve as metas na ordem do painel", () => {
    const { items } = call<{ items: LibraryItemMetaOut[] }>(
      "diagram_library_list",
      {},
    );
  ...
  it("diagram_library_remove apaga o item, broadcasta e devolve o restante", () => {
    ...
    const { n } = getDb()
      .prepare(
        `SELECT COUNT(*) AS n FROM diagram_library_items WHERE id = 'li-1'`,
      )
      .get() as { n: number };
    expect(n).toBe(0);
  });
