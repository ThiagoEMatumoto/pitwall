...
function objectiveTools(notify: McpNotify): ToolDef[] {
  return [
    ...
    },
    {
      ...
      description:
        "Update fields of an existing objective by id. Only provided fields change.",
    ...
    },
    {
      ...
      description:
        "Archive an objective (reversible soft-delete; it leaves active listings).",
      inputSchema: idSchema,
      handler: (args) => {
        const { id } = idSchema.parse(args);
    ...
    },
    {
      ...
      description:
        "Create a key result under an objective (objectiveId required).",
    ...
    },
    {
      ...
      description:
        "Update fields of an existing key result by id. Only provided fields change.",
...
function featureTools(notify: McpNotify): ToolDef[] {
  return [
    ...
    },
    {
      ...
      description:
        "Archive a feature (reversible soft-delete; it leaves active listings).",
      inputSchema: idSchema,
      handler: (args) => {
        const { id } = idSchema.parse(args);
...
function resolveRepo(ref: string): ResolvedRepo {
  ...
  if (rows.length > 1) {
    const candidates = rows
      .map(
        (r) =>
          `- label="${r.label}" path="${r.path}" project="${r.project_name}"`,
      )
      .join("\n");
    throw new Error(
...
): ReturnType<typeof getActivityFor> {
      ...
      handler: (args) => {
        ...
        const connections = repoDepStore.listByRepo(r.id).map((edge) => {
          ...
          return {
            ...
            kind: edge.kind,
            label: edge.label,
            direction: (outgoing ? "outgoing" : "incoming") as
              "outgoing" | "incoming",
    ...
    },
    {
      ...
      handler: (args) => {
        ...
        if (!input.force) {
          ...
          const mine = mother
            ? handoffStore.findActiveByTarget(target.id, mother)
            : null;
    ...
    },
    {
      ...
      handler: (args) => {
        ...
        if (!handoff.childSessionId) {
          throw new Error(
            `handoff ${handoffId} ainda não tem sessão-filha atrelada.`,
          );
        }
...
const jobPermissionMode = z.enum(OBSERVE_ONLY_PERMISSION_MODES, {
  error:
    "permissionMode autônomo indisponível via MCP no MVP — use a UI (apenas plan/default)",
});
...
const forbiddenFactStatus = z.enum([
  "proibido",
  "liberado",
  "confirmado-falso",
]);
const allowedFactScope = z.enum([
  "afirmavel",
  "condicional",
  "somente-com-fonte",
]);
  ...
  return [
    {
      ...
      handler: (args) => {
        ...
        const contract = id
          ? contentStore.get(id)
          : contentStore.getBySlug(slug as string);
        if (!contract)
          throw new Error(`content contract não encontrado: ${id ?? slug}`);
    ...
    },
    {
      ...
      handler: (args) => {
        ...
        if (!contract) {
          throw new Error(
            `content contract não encontrado: ${input.contractId ?? input.slug}`,
          );
        }
...
const skeletonElementType = z.enum([
  "rectangle",
  "ellipse",
  "diamond",
  "text",
...
const EXACTLY_ONE_MSG =
  "provide exactly one of elements (skeleton) or scene (raw Excalidraw scene)";
  ...
  return [
    ...
    },
    {
      ...
      handler: (args) => {
        ...
        const items = diagramStore
          .list(filter)
          .map(({ thumbnail: _thumb, ...meta }) => meta);
        return ok({ items });
      },
    },
    {
